...
const ContactUpload2 = () => {
  ...
  const fetchContacts = async () => {
    try {
      const res = await axios.get("http://localhost:3000/api/contact2/all");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("http://localhost:3000/api/contact2/create", formData);
      setName("");
      setDesignation("");
      setMobile("");
      setEmail("");
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:3000/api/contact2/delete/${id}`);
      fetchContacts();
    } catch (err) {
  ...
  const EditContactModal = ({ contact, onClose }) => {
    ...
    const handleUpdate = async () => {
      ...
      try {
        await axios.put(`http://localhost:3000/api/contact2/update/${contact._id}`, formData);
        onClose();
        fetchContacts();
      } catch (err) {
